...
export async function GET(request: Request) {
  ...
  const questionStats = new Map<
    string,
    { questionId: string; total: number; count: number; type?: string; text?: Record<string, string> }
  >();
  ...
  const questionRows = Array.from(questionStats.values()).map((stat) => ({
    questionId: stat.questionId,
    text: stat.text,
    type: stat.type,
    averageScore: stat.count > 0 ? stat.total / stat.count : 0,
    count: stat.count,
  }));
